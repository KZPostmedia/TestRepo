...
Postmedia.adConfig.defineAd = function(slot) {
	...
	if (adOop || adSize.length>0) { // Do if Oop ad or there are ad sizes
		...
		if (Postmedia.adConfig.adslots[slot].isCompanion) {
			...
			gptadslots[adName].addService(googletag.pubads());
		if (Postmedia.adConfig.enablerefresh && Postmedia.adConfig.adslots[slot].refresh && Postmedia.adConfig.Constants[Postmedia.adConfig.adslots[slot].type].refreshAllowed) {
...
Postmedia.adConfig.refreshAds = function(param, changeCorrelator){
	...
	if (slotnames.length>0) {
		for (i=0;i<slotnames.length;i++)
			googletag.cmd.push(function(){googletag.pubads().refresh([gptadslots[slotnames[i]]], opt)});
	}
	else
